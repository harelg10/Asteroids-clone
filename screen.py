...
class Screen:
    ...
    def _bind_keys(self):
        ...
        self._bind_key("a", self._torpedo_left)
        self._bind_key("d", self._torpedo_right)
        self._bind_key("v", self.tracking_torpedo)
        self._bind_key("f", self.handle_freeze)

    def tracking_torpedo(self):
        self.missileClick += 1

    def _torpedo_left(self):
        self._t_left += 1

    def handle_freeze(self):
        self.freezeClick += self.FREEZETIME

    def _torpedo_right(self):
        self._t_right += 1
    ...
    def is_torpedo_left_pressed(self):
        """
        :returns: True if the left key was pressed, else False
        """
        res = self._t_left > 0
        self._t_left -= 1 if res else 0
        return res

    def is_torpedo_right_pressed(self):
        """
        :returns: True if the left key was pressed, else False
        """
        res = self._t_right > 0
        self._t_right -= 1 if res else 0
        return res
    ...
    def is_v_pressed(self):
        ...
        :returns: True if the fire key was pressed, else False
        """
        res = self.missileClick > 0
        self.missileClick -= 1 if res else 0
        return res
    ...
    def is_f_pressed(self):
        ...
        :returns: True if the fire key was pressed, else False
        """
        res = self.freezeClick > 0
        self.freezeClick -= 1 if res else 0
        return res
